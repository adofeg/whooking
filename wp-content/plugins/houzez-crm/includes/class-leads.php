...
if ( ! class_exists( 'Houzez_Leads' ) ) {
	...
	class Houzez_Leads {
		...
		public static function get_leads() {
			global $wpdb;
            $table_name = $wpdb->prefix . 'houzez_crm_leads';
			$table_name2 = $wpdb->prefix . 'houzez_crm_leads_hide';
            $usuario=get_current_user_id();
			$where_nuevo="";

			if(isset($_GET['search-param'])){
				$parametro=sanitize_text_field( $_GET['search-param']);
			...
			//$query = 'SELECT * FROM '.$table_name.' WHERE user_id= '.get_current_user_id();//@original
			$query = "SELECT * FROM ".$table_name." WHERE ".$where_nuevo;//@adolph
		...
		public static function get_all_leads() {
			...
			$table_name2 = $wpdb->prefix . 'houzez_crm_leads_hide';
			$usuario=get_current_user_id();
			$extra_user="'".$usuario."' OR user_id='1'";//@adolph

            //$sql = "SELECT * FROM $table_name WHERE user_id= ".get_current_user_id();//@original
			$orden=" ORDER BY display_name ASC";
			$hide= " AND lead_id NOT IN (SELECT lead_id from ".$table_name2." WHERE (user_id='".$usuario."' or user_id='0'))";
			$sql = "SELECT * FROM $table_name WHERE (user_id= ".$extra_user.")".$hide.$orden;//@adolph
		...
		public function delete_lead() {
			...
			$usuario=get_current_user_id();
			$query="SELECT * FROM ".$table_name." WHERE lead_id ='".$lead_id."' AND user_id='".$usuario."'";
			...
			if(count($results)>0){
				...
				$wpdb->query("INSERT INTO ".$table_name2." VALUES('".$lead_id."','".$usuario."')");
			}
