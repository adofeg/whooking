...
if ( ! class_exists( 'Houzez_Enquiry' ) ) {
	...
	class Houzez_Enquiry {
		...
		public static function get_enquires() {
			//@adolph
			if(isset($_GET['area'])){
				$param_area=" AND enquiry_meta like '%".sanitize_text_field($_GET['area'])."%'";
			}

			if(isset($_GET['solicitud'])){
				$param_solicitud=" AND enquiry_type like '%".sanitize_text_field($_GET['solicitud'])."%'";
			}
			...
			$table_name2 = $wpdb->prefix . 'houzez_crm_enquiries_hide';
			$extra_user=" OR user_id='1')" ;
			$usuario=get_current_user_id();
            $andwhere = '';
			$hide= " AND enquiry_id NOT IN (SELECT enquiry_id from ".$table_name2." WHERE user_id='".$usuario."')";
			if(isset($_GET['lead-id']) && !empty($_GET['lead-id'])) {
				$andwhere = 'AND lead_id="'.$_GET['lead-id'].'"';//@original
				//$andwhere = 'WHERE lead_id="'.$_GET['lead-id'].'"';//@adolph
			}
            
            $items_per_page = isset($_GET['records']) ? intval($_GET['records']) : 10;
			$page = isset( $_GET['cpage'] ) ? abs( (int) $_GET['cpage'] ) : 1;
			$offset = ( $page * $items_per_page ) - $items_per_page;
			$query = 'SELECT * FROM '.$table_name.' WHERE (user_id= '.$usuario.$extra_user.' '.$andwhere.$param_area.$param_solicitud.$hide;//@original
			//$query = 'SELECT * FROM '.$table_name.' '.$andwhere;//@adolph
			...
			for($i=0;$i<count($results);$i++){
				$datos=maybe_unserialize($results[$i]->enquiry_meta);
				if(!empty($datos['area']['name'])){
					$results[$i]->zona=$datos['area']['name'];
				}
		...
		public function delete_enquiry() {
			global $wpdb;
            $table_name = $wpdb->prefix . 'houzez_crm_enquiries';
			$table_name2 = $wpdb->prefix . 'houzez_crm_enquiries_hide';//@adolph
	        ...
	        $extra_query=" AND user_id= ".$usuario;//@adolph
	        $wpdb->query("DELETE FROM {$table_name} WHERE enquiry_id IN ($ids)".$extra_query);
			foreach ($array_ids as &$valor) {
				$wpdb->query("INSERT INTO {$table_name2} VALUES('".$valor."','".$usuario."')");
			}
		...
		public function register_clicks($inquiry_id){

			global $wpdb;
			
			$results = $wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}houzez_crm_enquiries SET clicks = clicks + 1 WHERE enquiry_id = %d", $inquiry_id), ARRAY_A);
		...
		public function send_match_listing_email() {
			...
			$inquiry = sanitize_text_field($_REQUEST['inquiry']);
			register_clicks(inquiry);
