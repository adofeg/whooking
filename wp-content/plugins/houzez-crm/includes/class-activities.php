...
if ( ! class_exists( 'Houzez_Activities' ) ) {
	...
	class Houzez_Activities {
		...
		public static function get_activities() {
			...
			$reviews=self::not_reviews();
			$query = 'SELECT * FROM '.$table_name.$reviews;//@adolph
			//echo $query;
		...
		public static function not_reviews() {
			$sub_query="";
			...
			$args = array(
				...
				$agencias=$my_posts->posts;
				
				foreach ($agencias as $agencia){
					$agencias_ids[]=$agencia->ID;
				}

				foreach ($agencias_ids as $id){
					$sub_query.=" WHERE activity_id NOT IN (SELECT activity_id FROM qyc_houzez_crm_activities WHERE meta like '%s:4:\"type\";s:6:\"review\"%' and meta like '%\"listing_id\";i:".$id.";%')";
				}
				return $sub_query;
